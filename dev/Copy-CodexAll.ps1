...
$filenames = Get-ChildItem '/Volumes/`[C`] Windows 11 (ARM)/codex-windows/frontend' | Select-Object name | Where-Object {$_.name -ne "node_modules"}
...
$filenames = Get-ChildItem '/Users/amaula/GitHub/codex/frontend' | Select-Object name | Where-Object {$_.name -ne "node_modules"}
foreach($filename in $filenames) {
    Write-Output $filename.name
